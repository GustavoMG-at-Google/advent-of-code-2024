...
std::map<int32_t, std::set<std::pair<Coord, Coord>>> shortcuts;
void FindShortcuts() {
    ...
    for (int32_t col = 0; col < cols; col++) {
      for (int32_t row2 = row - LIM; row2 <= row + LIM; row2++) {
        for (int32_t col2 = col - LIM; col2 <= col + LIM; col2++) {
          ...
          Coord b{row2, col2};
          if (Shortcut(a, b)) {
            shortcuts[ShortcutSaving(a, b)].emplace(a, b);
          }
        }
      }
    }
...
int main() {
  ...
  FindShortcuts();
  int32_t ans = 0;
  for (const auto& [saving, shortcuts] : shortcuts) {
    if (saving >= 100) ans += shortcuts.size();
  }
